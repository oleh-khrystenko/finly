...
## [22. Білінг під керуванням нашого коду](22-self-managed-billing/README.md)

Перевертаємо модель рекурентності: життєвим циклом підписки (продовження, повторні спроби, зміна плану, скасування) повністю керує наш код через cron-обхід бази як єдине джерело правди, а WayForPay зводиться до хостованого checkout із захопленням токена, разового списання за токеном і повернення. Знімаємо всю машинерію нативних Regular Payments (провайдерський розклад, suspend / resume / remove / change, чергу видалень рекуренту). Продовження стає синхронним списанням за токеном, збій картки веде в dunning зі збереженням доступу на грейс-вікно і блоком після вичерпання спроб, апгрейд списує доплату тихо без редиректу, зміна картки спрощується до повторного захоплення токена без особистого кабінету WayForPay і телефону. Каталог, рівні доступу і реконсиляція зі Sprint 19 не змінюються. Живих підписок немає, тож чистий старт без міграції.

**Status:** заплановано (2026-06-22), не стартував. Блокер дня один: запит у підтримку WayForPay на токенізацію і захоплення токена без реального списання (lead-time невідомий).
