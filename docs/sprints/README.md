...
>
> **Перенумерація 2026-08-23.** Порядок спринтів епіку документів приведено до порядку виконання: видалення акаунта стало 31 (було 32), сховище документів 32, далі 33..36 конвеєр, словник, кабінет і пошуковий двигун, 37 злиття записів (було 31), 38 запрошення власника, 39 юридика і довідка. Злиття поїхало вниз, бо не має живого сценарію до появи запрошень і мусить уміти переносити документи. Слоти 33..36, 38, 39 поки заглушки. Номер спринту в коментарях коду оновлено разом з нумерацією.
>
...
Міняємо провайдера на monobank «Плата» і перевертаємо модель рекурентності. monobank не має власного рекуренту, тож життєвим циклом підписки повністю керує наш код через cron-обхід бази (billing clock) як єдине джерело правди, а monobank дає лише хостований checkout із захопленням токена, разове списання за токеном і запит статусу транзакції. Знімаємо всю машинерію WayForPay Regular Payments. Скоуп звужено до MVP: 2 підписки, 2 one-off, скасування лише в кінці періоду; збій картки веде в dunning зі збереженням доступу на грейс-вікно (10 добових спроб) і відновленням через дію «оплатити зараз». Зрізано зміну тарифу, окрему зміну картки, негайне скасування з поверненням і refund будь-де в коді. Каталог, рівні доступу і реконсиляція зі Sprint 19 не змінюються. Живих підписок немає, тож чистий старт без міграції.

**Status:** реалізовано, код у `main` (модуль monobank, billing clock, dunning; машинерію WayForPay знесено). Тарифну модель поверх цього фундаменту переписав Sprint 27. Межу «зміна картки поза прострочкою» знімає Sprint 43.
...
**Status:** архітектура затверджена (2026-07-07); плани окремих спринтів створюються далі по одному. Передумова до решти епіку — [`research-spike.md`](26-documents-architecture/research-spike.md) (пошуковий двигун, вектори, безпека зберігання) — **виконана 2026-08-06**, висновки внесені в архітектуру. Головне: прод на MongoDB Atlas (припущення «без Atlas» було хибним), тож окремого пошукового двигуна на VPS не буде — повнотекст і вектори живуть у тій самій базі; української морфології немає ні в кого з легких варіантів і її бере на себе семантика; документам потрібен окремий приватний бакет з юрисдикцією ЄС, бо нинішнє сховище публічне за посиланням. Наслідок — [`research-spike-2-vectors-and-redundancy.md`](26-documents-architecture/research-spike-2-vectors-and-redundancy.md) — **виконано 2026-08-09**: вектори рахує прямий Voyage з відмовою від навчання (переговори не потрібні, угода публічна і діє автоматично), власна модель на сервері — друга хвиля; друга копія файлів — Backblaze B2 у Європі з фоновим копіюванням і відкладеним видаленням. Після plan-review (2026-08-10) закриті ще дві розвилки: нульовий баланс кредитів (резерв наперед + стан «чекає кредитів», рента сховища через грейс-вікно, ніякого мінусу) і прийняття запрошення при конфлікті податкового номера. Третій plan-review (2026-08-11) закрив три: отримувача з непорожнім архівом видалити не можна (спершу окрема дія «видалити всі документи» з підтвердженням, те саме правило на видаленні акаунта); середовище пошуку — під розробку заводиться окремий Atlas-кластер, а самі повнотекст і семантика свідомо лишаються поза автотестами і перевіряються руками за чеклістом; і головне — рішення про конфлікт податкового номера **перевернуто**: два записи одного ФОПа в системі не лишаються ніколи (у людини не може бути двох ФОПів з одним номером — це закон), конфлікт розв'язується злиттям записів, яке стало окремим спринтом [37](38-business-merge/planning-questions.md) і передумовою запрошень. Четвертий plan-review (2026-08-11) закрив два наслідки цих рішень: **вихід ≠ видалення** — над отримувачем, за яким стоїть ще хтось, людина відмовляється від прав (бухгалтер виходить зі списку менеджерів, власник обнуляє поле власника), архів лишається тим, хто працює далі, а заборона на видалення при непорожньому архіві діє тільки для останнього; і **власність запрошеному передається на завершенні онбордингу, а не на прийнятті запрошення** — інакше чинне прибирання покинутих профілів забирало б бухгалтерського отримувача з архівом за незаповнене імʼя, а перевірка конфлікту податкового номера тепер спрацьовує у двох точках (нічийні записи на прийнятті, власні — на онбордингу).
...
**Status:** реалізовано (код у `develop`).
...
## [30. Спільна сесія кабінету і pay-хоста, дані платника у профілі](30-shared-session-payer-profile/README.md)

Сесія переїжджає на батьківський домен: один вхід видно і в кабінеті, і на pay-хості, публічні сторінки отримують шапку кабінету (вхід далі відбувається в кабінеті з поверненням назад). Поверх цього податкова сторінка перестає вимагати ручний ввід: у профілі зʼявляються власні податкові дані (по батькові, РНОКПП) і список платників під сценарій бухгалтера, підстановка завжди редагована, а для залогіненого персональні значення більше не осідають в адресному рядку. Анонімний сценарій лишається незмінним.

**Status:** реалізовано (код у `develop`).

## [31. Видалення акаунта: доля отримувачів](31-account-deletion-cascade/README.md)

Закриття прогалини, знайденої при plan-review епіку документів: прибирання видалених акаунтів ніколи не торкалось отримувачів (написане у спринті 1, коли їх ще не існувало), тож після людини лишались записи з посиланням на неіснуючого власника і живими публічними сторінками. Спринт вирівнює і запобіжники: видалення акаунта отримує той самий шаблон підтвердження, що ручне видалення отримувача (вписати кількість вкладеного) — на обох наявних шляхах підтвердження, паролем і листом, які тепер відбуваються після перегляду переліку, а не замість нього; публічність гасне в момент підтвердження (не натискання кнопки — інакше чужа рука за відкритим ноутбуком гасила б платіжні сторінки) при збереженні вікна відновлення; прибираються решта хвостів людини, причому платіжний профіль знищується лише після перерахунку доступів; плюс разова чистка вже накопичених сиріт.

**Status:** реалізовано (код у `develop`). Доопрацювання (вибір долі отримувачів, за якими стоїть ще хтось) поїхало у спринт 39: сьогодні такого стану не буває — механізму додати до запису другу людину немає, тож видалення акаунта забирає геть усе своє. Правило «отримувача з непорожнім архівом не можна видалити» знято 2026-09-03: сховище кріпиться до акаунта, тож архів на записі отримувача не висить.


## [32. Сховище документів](32-documents-storage/README.md)

Другий спринт епіку документів і перший, що торкається файлів. Головне рішення, ухвалене при плануванні і таке, що скасовує попереднє: сховище кріпиться до людини, а не до отримувача. У кожного акаунта своє кореневе сховище, документи в ньому згруповані за отримувачами, а господар документа — той, хто завантажив файли, і це не змінюється. Наслідок: чужих документів усередині отримувача не існує в принципі, тож немає ні архівних отримувачів, ні заборони видаляти отримувача з непорожнім архівом, ні розвилки з долею документів при виході — коли отримувач зникає, з чужих документів просто знімається позначка. Спринт дає приватний бакет з юрисдикцією ЄС, документи і файли, нормалізацію фото зі стисканням, жорсткий ліміт 5 ГБ на акаунт, кошик і журнал доступу; їде в темряві — продаж документного пакета тут не вмикається, доступ на час спринту службовий. Теки і спільний доступ винесено у спринт 33, увесь AI у спринт 34, докупівля обсягу — спринт 42.

**Status:** заплановано (2026-08-28, модель переглянута 2026-09-03), не стартував.

## [33. Теки і спільний доступ до документів](33-documents-sharing/README.md)

Теки як сутність (створити, назвати, вкласти, перенести документи) — переїхали сюди зі спринту 32 (2026-09-03). Поверх них спільна робота: три рівні прав (дивитись, скачати, видалити), бейдж прав біля назви теки, один документ одночасно в кількох теках, вступ нового учасника і зміна прав за згодою всіх, чиї документи в теці лежать, вихід учасника без чиєїсь згоди.

**Status:** заглушка, плану ще немає.

## [34. AI-конвеєр обробки документів](34-ai-pipeline/README.md)

Черга фонової обробки, канонічна «ідеальна копія» вмісту, визначення типу, витяг полів і тегів з чанкуванням словника, впевненість, оцінка вартості з резервом кредитів і станом «чекає кредитів», списання за фактом, прогрес партій, режим «без обробки».

**Status:** заглушка, плану ще немає.

## [35. Словник тегів](35-tag-vocabulary/README.md)

Глобальний курований словник у конфігу репозиторію, увімкнена підмножина на рівні отримувача, пропозиції нових тегів від AI і від людей, звʼязок «тип документа визначає релевантні теги».

**Status:** заглушка, плану ще немає.

## [36. Пошук і кабінет документів](36-documents-cabinet/README.md)

Глобальний розділ «Документи» як робочий стіл бухгалтера і той самий екран усередині отримувача, фільтри по полях і тегах, черга перевірки, ручна правка з пріоритетом над AI, попередження про дублікати, сторінка кошика.

**Status:** заглушка, плану ще немає.

## [37. Пошуковий двигун](37-search-engine/README.md)

Індекси повнотексту і векторів у тій самій базі (Atlas), нормалізація української при індексації без морфології, векторизація фрагментів канонічних копій, запити з ізоляцією по отримувачах із guard-резолву. Першим кроком — окремий Atlas-кластер під розробку і чекліст ручної перевірки у `docs/manual-checks/`, бо обидва рівні свідомо поза автотестами.

**Status:** заглушка, плану ще немає.

## [38. Злиття записів отримувача](38-business-merge/planning-questions.md)

Спринт епіку документів, породжений plan-review 2026-08-11. У людини не може бути двох ФОПів з одним податковим номером, тож два записи того самого реального отримувача — не робочий стан, а розбіжність, яку треба звести в один запис: реквізити, рахунки, документи, історія імен, публічність і бренд переїжджають у переможця, друга публічна адреса закривається. Конфлікт буває двох видів (власний запис клієнта проти нічийного запису бухгалтера і два нічийні записи різних бухгалтерів — типово після зміни бухгалтера), обидва стережуть partial-unique індекси в базі. Передумова спринту запрошень власника: без злиття прийняття запрошення при конфлікті ні виконати, ні чесно відхилити не можна.

**Status:** ініційовано (2026-08-11) — заведені питання для архітектури, відповідей ще немає, плану ще немає. Перенумеровано 2026-08-23 (був 31): злиття не має живого сценарію до появи запрошень і мусить уміти переносити документи, тож його місце — після сховища і конвеєра, перед запрошеннями.


## [39. Запрошення власника у спільний отримувач](39-owner-invitations/README.md)

Бухгалтер запрошує клієнта, клієнт реєструється і бачить той самий запис: доступ дає прийняття, власність — завершення онбордингу. Перевірка конфлікту податкового номера у двох точках, вхід у злиття записів при конфлікті, бухгалтер лишається менеджером.

**Status:** заглушка, плану ще немає.

## [40. Юридика і довідка епіку документів](40-legal-and-help/README.md)

Фінальний спринт епіку і блокер запуску: друга редакція умов і політики конфіденційності під зберігання чужих документів (обидва AI-постачальники поіменно, юрисдикція сховища, строки видалення, резервний постачальник, режим «без обробки»), підняття версії умов, публічна сторінка «Безпека», хвиля help-статей під новий функціонал.

**Status:** заглушка, плану ще немає.

## [41. Бренд купує лише власник](41-brand-owner-only/README.md)

Прикріплення отримувача до складу «Бренд» дозволене лише власнику запису. Сьогодні це може зробити будь-хто з доступом, включно з бухгалтером над чужим або нічийним записом — сценарію під це немає, а станів воно породжує багато. Заведено 2026-09-03 при плануванні спринту 32; до епіку документів не належить, за порядком виконання може йти раніше за номер.

**Status:** заглушка, плану ще немає.

## [42. Докупівля обсягу сховища](42-storage-volume/README.md)

Обсяг понад базові 5 ГБ: що означає розмір документного пакета, чи діє формула «5 ГБ × розмір пакета» з тарифної сітки, щомісячна рента кредитами за перевищення і грейс-вікно при нестачі. У спринті 32 ліміт свідомо жорсткий, без ренти. Місце за порядком виконання — після кабінету документів (спринт 36).

**Status:** заглушка, плану ще немає.

## [43. Життєвий цикл платіжної картки і оборотне скасування](43-payment-method-lifecycle/README.md)

Скасування підписки сьогодні знищує збережену картку разом з наміром не поновлювати, тож платник на весь залишок оплаченого періоду втрачає платні дії, не може відкотити скасування (дії немає) і не може вписати іншу картку (заміни немає в системі взагалі, навіть для активного платника). Спринт розчіплює платіжний інструмент, намір поновлення і право доступу, додає відновлення скасованої підписки і прив'язку картки через верифікаційний рахунок monobank без списання. Заразом переглядає межу Sprint 22, який виніс заміну картки у НЕ-скоуп через відсутність дешевого шляху: дослідження 2026-09-09 показало, що верифікація без списання доступна.

**Status:** реалізовано (2026-09-10), код у гілці розробки. Лишились ручні перевірки на живих грошах (PAY-7..PAY-11) і рядок про строк зберігання картки у другій редакції умов.
