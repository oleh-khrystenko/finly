...
> **Статус:** working draft, 2026-05-03.
...
- 🔲 **`seoIndexEnabled`** (рішення E3).
  - Boolean, default `false`. Public-view **повертає** це поле (whitelist C4 розширюється на 5-те поле — `seoIndexEnabled`), бо public Server Component читає його напряму для рендеру `<meta name="robots">`.
...
- 🔲 **Контракти write-side у `packages/types/src/contracts/businesses.ts`** (новий файл):
  ...
  - `PublicBusinessSchema` — view-схема для public endpoint: `{ type, name, slug, acceptedBanks, seoIndexEnabled }`. Single source of truth для frontend-фетчу.
...
**DoD:** Mongoose-схема приймає нові поля; entity-Zod і write-DTO дотримуються coupled-правил; public-DTO whitelist'ить точно 5 полів; existing Sprint 1 тести залишаються зеленими (нові поля required mongoose-default-ами не ламають instantiation).
...
- 🔲 **GET `/businesses/public/:slug`**.
  ...
  - Response — **тільки whitelist-поля** (рішення C4 + розширення на `seoIndexEnabled` для frontend-рендеру meta-тегу): `type`, `name`, `slug` (canonical case-preserved), `acceptedBanks`, `seoIndexEnabled`. Жодних `iban`, `taxId`, `taxationSystem`, `isVatPayer`, `paymentPurposeTemplate`, `ownerId`, `managers`, timestamps.
  - **Реквізити доступні клієнту тільки через QR** — це навмисний інваріант (C4): single source of truth для leak-сурфейсу.
...
**DoD:** public endpoint віддає рівно 5 полів (тест на whitelist — `expect(response.body.data).toHaveProperty(...)` для 5 ключів і `not.toHaveProperty(...)` для решти 8); 404 при невідомому slug; обидва QR-endpoint-и віддають valid PNG (jsqr round-trip декодує payload-link, або просто test-fixture buffer length > 0); `Cache-Control: public` headers присутні; немає `Authorization` requirement-у — e2e тест без bearer проходить.

**TPM-ризик:** **public-endpoint раніше або пізніше стане ціллю scraping/probing** (бот-перебір slug-ів для виявлення активних бізнесів і phishing). Sprint 3 не закладає захист, бо attack-surface обмежений — leak-fields whitelist — `name + slug + acceptedBanks + seoIndexEnabled`, всі вони public-by-design. Реквізити (IBAN, ІПН) недоступні через JSON, але **видимі через QR** будь-якому досягнувшому slug (це властивість продукту, не баг). У Phase 1.5+ (коли з'являться інвойси з сумами) — додаємо per-IP slug-rate-limit (sliding window) і потенційно CAPTCHA на enumerate-патерн. Не блокер MVP.
...
- 🔲 **Server-side fetch + 404 + canonical redirect.**
  ...
  - Якщо знайдено, але URL case ≠ canonical → `permanentRedirect('/{canonicalSlug}')` (E1: 301 на канонічну форму).
  - Дані для рендеру — `PublicBusinessSchema` (5 полів: type, name, slug, acceptedBanks, seoIndexEnabled).
- 🔲 **SEO / `<meta>`**:
...
- 🔲 **`CLAUDE.md`** — оновити секції:
  ...
  - "## Known Complexities" — додати:
    - "**Slug case-preserved + uniqueness on lower:** Twitter/Instagram-style. Unique-index на `slugLower`, не на `slug`. 301 redirect на canonical case при URL mismatch. Reserved-перевірка на lowercase." (E1)
    ...
    - "**Public endpoint whitelist (5 полів):** type, name, slug, acceptedBanks, seoIndexEnabled. Реквізити тільки через QR — single source of truth для leak surface." (C4)
