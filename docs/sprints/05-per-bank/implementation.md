...
- **Формат:** квадратний WebP 128×128, по одному файлу на банк
  (`apps/web/public/banks/<bankCode>.webp`). Будь-яке вхідне джерело
  нормалізується до цього єдиного спеку → консистентна сітка.
- **Джерело:** офіційні іконки застосунків з App Store через Apple iTunes
  Lookup API (`artworkUrl512`), bundleId — з НБУ-AASA. Канонічно, не випадкові
  картинки.
- **Генератор:** `apps/api/scripts/generate-bank-logos.ts`
  (`pnpm --filter api ts-node scripts/generate-bank-logos.ts`) — резолв
  bundleId → завантаження → `sharp` resize/webp → запис у `public/banks/`.
  Коміт: `.webp` + скрипт (reproducibility). Заглушки-SVG
  (`shared/icons/banks/`) видалено разом з `BANK_DISPLAY`.
- **Рендер:** єдине джерело істини — `shared/ui/UiBankLogo` (плейн-`<img>` на
  `/banks/<bankCode>.webp`, як `UiQrImage`; next/image зайвий для крихітного
  same-origin asset). Споживачі — `UiBankAppGrid` (банк-чузер) і `LandingBanks`
  (trust-rail на лендінгу); власних плейсхолдерів жоден не тримає.
