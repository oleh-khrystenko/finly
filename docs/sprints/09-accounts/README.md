...
- **Ризик 1 — точний реєстр МФО для 11 банків.** МФО може мінятись (банк змінив корпоративну структуру; SportBank/IZIBank через Альфа Bank має нюанси). Mitigation: при імплементації перевірити з NBU public registry на момент coding-у; на unknown МФО — graceful fallback "Банк •last4" (не throw). Tech-backlog ticket "Quarterly NBU MFO registry sync".
