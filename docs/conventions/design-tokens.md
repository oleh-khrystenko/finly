...
| ------------------- | ----- | ------------------------------------------------------------ |
| `text-sm`           | 14    | **підлога** — лейбли, хінти, eyebrow, вторинний текст, помилки |
| `text-base`         | 16    | body, абзаци                                                 |
...
4. **Ієрархія, не розмір заради розміру.** Eyebrow/kicker — найменший (`text-sm`), а не більший за значення під ним; лейбл тихіший за своє значення (`text-sm` muted над `text-lg` foreground).
5. **Шкала живе у shared-примітивах.** Розміри лейблів/хінтів/помилок задають `UiInput`, `UiTextarea`, `UiRadioCardGroup`, `UiEditableField` — не локальні класи сторінок. Сторінки споживають готову шкалу. Reference-implementation — `app/(protected)/business/[slug]/page.tsx`.
