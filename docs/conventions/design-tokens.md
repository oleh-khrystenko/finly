...
| `text-lg`   | 18  | заголовок секції (h2, `UiSectionCard`), значення поля (read-view), lead |
| `text-xl`   | 20  | орг.-правова форма у блоці назви (виняток, п. 5)                        |
| `text-2xl`  | 24  | заголовок сторінки (h1, `UiPageHeading`)                                |
| `text-xs`   | 12  | **резерв** — лише рідкісні щільні блоки, не за замовчуванням            |

Шкала описує **робочі поверхні** (кабінет, адмінка, форми). Її стеля — 24px, і
заголовки не ростуть на `md:`/`lg:`: на широкому моніторі сторінка отримує більше
**колонок**, а не більший кегль (двоколонковий layout деталок, auto-fill сітки
карток). Маркетингові поверхні (лендінг, публічні pay-сторінки) живуть за власною
display-шкалою (`text-4xl`+) — вони продають, а не тримають робочий потік.
...
6. **Однаковий кегль ≠ однаковий рівень.** Заголовок секції і значення поля обидва на `text-lg` (18) — розрізняє їх вага і трекінг (`font-semibold tracking-tight` у h2 проти звичайної ваги значення), а не розмір. Не «розводити» їх кеглем: шкала і так стисла, зайвий щабель зʼїсть контраст із h1.
7. **Шкала живе у shared-примітивах.** Розміри лейблів/хінтів/помилок задають `UiInput`, `UiTextarea`, `UiRadioCardGroup`, `UiEditableField` — не локальні класи сторінок. Сторінки споживають готову шкалу. Reference-implementation — `app/(protected)/business/[slug]/page.tsx`.
