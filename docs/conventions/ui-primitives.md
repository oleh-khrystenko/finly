...
| `UiDropdownMenu`     | Context menus, action menus, option pickers                                                                                 | align: `start`/`end`; side: `top`/`bottom`; sizes: sm/md/lg                                                                                                    |
...
| `UiDangerGateFields` | Той самий запобіжник без власного вікна: cloze-фраза з inline-полями, яку ставлять кроком усередині більшої модалки (видалення акаунта: перелік → кількості → пароль)      | -- ; контрольований (`values` + `onChange(index, value)`); `gates: { label, expected }[]` + `renderPrompt(input)`; звірка — `areDangerGatesSatisfied(gates, values)`; живе поруч з `UiDangerGateDialog`, який на ньому й побудований |
