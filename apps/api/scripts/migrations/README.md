...
| `2026-06-18-businesses-brand-null.ts`    | 2026-06-18 | Sprint 21: backfill `Business.brand = null` для pre-Sprint-21 бізнесів. `brand` — новий required-nullable субдок; mongoose `default: null` спрацьовує лише на гідрації окремого документа, а aggregation-список отримувачів (`$addFields`/`$unset`, не `$project`) успадковує форму документа як є — без backfill-у `brand` у списку був би undefined попри entity-контракт. Працює лише по `$exists: false` — idempotent.                                                                                                                                    | ⬜      | ⬜         |
| `2026-07-21-publicity-defaults.ts`       | 2026-07-21 | Sprint 29: backfill прапорів каталогу і публічності. `Business` (`isSystem`, `catalogVisible`, `publicityStatus` + поля життєвого циклу заявки, `catalogCategory`) і `Account` (`catalogVisible`, `paymentPurposeTemplate`) отримали Mongoose-дефолти, які застосовуються лише на insert. Read-шляхи це переживають (`$ifNull` в aggregation), а write-фільтр за станом — ні: `findOneAndUpdate({ publicityStatus: 'none' })` не матчив документ без поля, і подання заявки на публічність падало 409 назавжди. Працює лише по `$exists: false` — idempotent. |
| `2026-07-30-drop-user-tax-profile.ts`    | 2026-07-30 | Знесення власного РНОКПП з профілю: джерелом підстановки на податковій сторінці стали отримувачі-фізособи користувача (`/payer-sources`), тож `profile.taxId` — друга копія того самого номера, яка розходиться після редагування отримувача. Прибирає `profile.taxId` і стемп `taxProfilePromptDismissedAt` (пропозиція заповнити дані зникла разом з полем). Не «просто зняти зі схеми»: це податковий номер живої людини, який інакше лишився б у сховищі й бекапах без способу його побачити чи видалити. Працює лише по `$exists: true` — idempotent. |
