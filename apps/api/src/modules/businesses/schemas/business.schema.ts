...
export class Business {
    ...
    /**
     * Sprint 32 — публічність призупинена разом з деактивацією акаунта людини,
     * яка веде цей запис. Ставиться в момент ПІДТВЕРДЖЕННЯ видалення акаунта
     * (не на натисканні кнопки), знімається при відновленні, зникає разом з
     * записом при остаточному прибиранні.
     *
     * Стан на отримувачі, а не перевірка власника при читанні: публічна
     * сторінка анонімна і кешована, вона не має права робити зайвий запит по
     * власнику на кожен показ.
     *
     * Інваріант: прапор стоїть лише на записах, які підуть на видалення разом з
     * людиною. Запис, за яким лишається ще хтось, не гасне ніколи.
     *
     * Публічне читання приходить з lookup-ом по `slugLower`, каталог і sitemap
     * додають предикат понад власні partial-індекси, гасіння/повернення ходять
     * по `ownerId`/`managers` — там окремий index не потрібен. Веде вибірку сам
     * лише компенсаційний прохід (`CleanupService.resyncRestoredAccounts`), і
     * саме під нього стоїть partial-index нижче: у ньому лежать тільки
     * призупинені записи, тобто у штатному стані він порожній.
     */
    @Prop({ type: Date, default: null })
    publicitySuspendedAt!: Date | null;
...
// шукає позначки, що пережили відновлення акаунта. Partial, а не sparse: дефолт
// поля — `null`, тобто значення присутнє, і sparse тримав би кожен бізнес
// продукту. З partial-фільтром у індексі лежать самі лише призупинені записи —
// у штатному стані він порожній, і прохід не платить COLLSCAN.
BusinessSchema.index(
    { publicitySuspendedAt: 1 },
    { partialFilterExpression: { publicitySuspendedAt: { $type: 'date' } } }
);
