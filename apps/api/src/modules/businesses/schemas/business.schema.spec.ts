...
describe('Business schema (Mongoose integration)', () => {
    ...
    it('lowercases slug on insert', async () => {
        const doc = await BusinessModel.create(
            buildFixture({ slug: 'IVANENKO-FOP' })
        );
        expect(doc.slug).toBe('ivanenko-fop');
    });

    it('creates expected indexes (slug unique, ownerId sparse, managers)', async () => {
        const indexes = await BusinessModel.collection.indexes();

        const slugIdx = indexes.find((i) => i.key.slug === 1);
    ...
    it('rejects duplicate slug with MongoServerError code 11000', async () => {
        await BusinessModel.create(buildFixture());
