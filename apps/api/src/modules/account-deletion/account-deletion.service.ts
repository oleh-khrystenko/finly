...
/**
 * Sprint 31 — доля отримувачів при видаленні акаунта.
 *
...
export class AccountDeletionService {
     ...
     * (`nextChargeAt`/`nextRetryAt` → null) і ставить CANCELED. Картку тут не
     * стираємо: відкликати її у гаманці monobank уміє лише білінг, і погашений
     * профіль з карткою він підбирає погодинним стиранням (див. `purgeUser`).
     ...
     *
     * Повертає `false`, якщо реконсиляція не завершилась або збережену картку
     * ще не відкликано: людина і погашений профіль лишаються до наступного
     * проходу, який доробить і видалить. Ні недороблена реконсиляція, ні
     * невідкликана картка ніколи не є приводом знищити профіль.
     *
     * Чекання на картку кінечне: після межі відмов банку білінг сам виводить
     * токен з черги і сповіщає ops (`BILLING_CARD_REVOCATION_MAX_FAILURES`).
     * Інакше недоступність банку блокувала б видалення акаунта безстроково.
     *
    ...
    async purgeUser(userId: string): Promise<boolean> {
        ...
        // Картку у гаманці monobank відкликає білінг погодинним стиранням
        // карток погашених профілів і повтором відкликань, які банк не прийняв.
        // Знищений раніше за це профіль забрав би єдиний запис токена — і
        // збереженого, і того, що чекає у черзі, — і картка лишилась би в
        // гаманці назавжди. Затримка обмежена: банк, що вперто відмовляє,
        // вичерпує межу спроб, після якої білінг спорожняє чергу сам. Здане
        // відкликання теж тримає профіль, поки ops не отримав про нього лист:
        // мітка листа — останній слід картки, що лишилась у гаманці.
        const cardPending = await this.profileModel
            .exists({
                userId: userObjectId,
                $or: [
        ...
        if (cardPending) {
            this.logger.warn(
                `Saved card of user ${userId} not revoked yet — billing profile ` +
                    'kept retired, purge deferred to next pass'
            );
            return false;
        }
