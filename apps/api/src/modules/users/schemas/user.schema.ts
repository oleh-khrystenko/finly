...
class UserProfileData {
     ...
     * лише тому, хто платить податки через публічні сторінки каталогу.
     * Читається завжди разом з профілем (`getMe`), тож окремий index не
     * потрібен.
     *
     * Власного РНОКПП тут немає свідомо: податковий номер людини живе на її
     * отримувачі-фізособі і читається звідти (`/payer-sources`).
     */
    @Prop()
    middleName?: string;
}
...
export class User {
    ...
    /**
     * Sprint 32 — «остаточне прибирання почалось». Ставиться однією атомарною
     * операцією на початку hard-delete (`CleanupService`) під умовою «вікно
     * відновлення справді минуло» і більше не знімається: за нею вже знесені
     * отримувачі, і повертати нема чого.
     *
     * Це замок між двома сторонами, а не інформаційне поле. Прохід прибирання
     * читає список акаунтів раз, а потім довго ходить по ньому (на кожного
     * отримувача окрема транзакція), тож без замка людина, яка натисне
     * «Відновити» вже після читання списку, повернулась би у живий, але
     * порожній акаунт: `restore` зняв би деактивацію, а прохід усе одно доніс
     * би видалення до кінця. Тепер обидві сторони змагаються за один документ:
     * хто перший, той і виграв.
     */
    @Prop({ type: Date, default: null })
    accountPurgeStartedAt!: Date | null;
