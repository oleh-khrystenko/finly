 ...
 * Жодного suspend/resume/remove/change/refund рекуренту (їх знесено).
 * Sprint 43 — єдиний виняток: прив'язка картки у прострочці одразу списує борг,
 * тож під локом можуть бути ДВА виклики — звірка статусу рахунку прив'язки і
 * списання боргу (≤40s). Усе, що стан підписки не змінює (відкликання старої
 * картки у гаманці, лист про зміну картки), свідомо виконується вже після
 * звільнення лока, а фонове відкликання тримає лок на один виклик. TTL мусить
 * покривати ці виклики плюс Mongo-операції (зокрема webhook-TX) з запасом, інакше лок
