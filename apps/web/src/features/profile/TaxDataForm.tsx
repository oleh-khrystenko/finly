...
/**
 * Поле опційне і очищуване: порожній рядок — це «прибрати значення», і саме так
 * його трактує сервер. Тому валідатор застосовується лише до непорожнього
 * вводу — інакше порожня форма (звичайний стан для того, хто податків не
 * платить) світилася б помилкою.
 */
...
/**
 * Sprint 30 — власний податковий номер. Джерело підстановки на публічних
 * податкових сторінках: заповнив один раз — далі поля на сторінці оплати
 * заповнюються самі (і лишаються редагованими перед оплатою). ПІБ, включно з
 * по батькові, береться з особистих даних — окрема секція тут саме тому, що
 * РНОКПП, на відміну від імені, потрібен лише для податків.
 */
export default function TaxDataForm({ user }: Props) {
    ...
    const onSubmit = async (data: TaxDataFormValues) => {
        try {
            await updateProfile({ taxId: data.taxId.trim() });
            const me = await getMe();
            setUser(me);
            form.reset({ taxId: me.profile.taxId ?? '' });
    ...
    return (
        <UiSectionCard title="Податкові дані">
            <p className="text-muted-foreground mt-2 text-sm">
                Потрібен лише для сплати податків через Finly: ми підставимо
                його у призначення платежу замість ручного вводу. Цей номер
                бачите тільки ви.
            </p>
