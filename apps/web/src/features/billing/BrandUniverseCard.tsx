...
export default function BrandUniverseCard({
    ...
    // Скасований-до-кінця-періоду профіль: доступ і картка живі, але платні дії
    // свідомо заблоковані (BILLING_CANCEL_PENDING) — куплений слот згас би
    // разом з підпискою на межі періоду. Показуємо безкоштовні дії і ведемо на
    // відновлення, після якого платні дії працюють як звичайно.
    ...
    const handlePick = (businessId: string) => {
        ...
        if (cancelPending) {
            toast.error(
                'Підписку скасовано. Відновіть її, щоб додати слот'
            );
            return;
        }
            ...
            {capacity === 0 ? (
                <div className="mt-6 space-y-4">
                    {cancelPending ? (
                        ...
                        // підключення це платна дія, а вони на скасованій
                        // підписці заблоковані. Шлях один і той самий —
                        // відновити підписку.
                        <p className="text-muted-foreground text-sm">
                            Підписку скасовано. Відновіть її, щоб підключити
                            Бренд
                        </p>
                    ) : (
                        <>
                ...
                <div className="mt-6 space-y-4">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            {/* На скасованому профілі новий слот купити не
                                можна (платні дії заблоковані), тож кнопка
                                лишається лише поки є вільні слоти для
                                безкоштовного прикріплення. */}
                    ...
                    {scheduledCapacity !== null && (
                        <div className="border-border bg-muted/40 flex flex-wrap items-center justify-between gap-3 rounded-lg border p-4">
                            ...
                            {/* На скасованому профілі відкликання зменшення —
                                білінг-мутація, яку API відхиляє разом з рештою
                                платних дій; спершу відновлення підписки. */}
                            {!cancelPending && (
                                <UiButton
                                    variant="outline"
                                    size="sm"
