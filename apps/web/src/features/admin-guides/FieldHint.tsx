...
export function FieldHint({ children }: { children: ReactNode }) {
    return (
        <div className="text-muted-foreground mt-1.5 flex gap-1.5 text-xs leading-relaxed">
            ...
            <div className="[&_code]:bg-muted [&_strong]:text-foreground min-w-0 space-y-1.5 [&_code]:rounded [&_code]:px-1 [&_code]:py-0.5 [&_code]:text-[0.9em] [&_strong]:font-medium [&_ul]:list-disc [&_ul]:space-y-1 [&_ul]:pl-5">
                {children}
            </div>
        </div>
    );
