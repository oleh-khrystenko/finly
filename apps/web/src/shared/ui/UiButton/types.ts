...
interface BaseProps {
    ...
    loading?: boolean;
    /**
     * Ховає текстовий лейбл на вузьких екранах (`hidden sm:inline`), лишаючи
     * тільки іконку — для toolbar-кнопок, що на mobile стають icon-only.
     * Клас вішається на **власну** обгортку лейбла, тож на mobile вона зникає
     * з flex-потоку і `gap-2` не резервує простір (інакше лишався б
     * несиметричний відступ збоку від іконки). Обов'язково парний з `aria-label`
     * — коли текст візуально схований, доступну назву несе саме він.
     */
