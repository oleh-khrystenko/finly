...
- **QR asset shipping**: `apps/api/src/modules/qr/assets/finly-logo-qr.png` копіюється у `dist/modules/qr/assets/` через `nest-cli.json` `compilerOptions.assets`. `QrService` резолвить шлях через `__dirname` — однаково працює і в dev (ts-node), і в prod (compiled `dist/`).
